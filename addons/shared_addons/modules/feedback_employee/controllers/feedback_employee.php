...
class Feedback_employee extends Public_Controller
{
    ...
    public function __construct() {
        ...
        $this->load->model('feedback_employee_m');
    ...
    public function index()
    {
        ...
        $total_rows = $this->feedback_employee_m->count_by($base_where);
        $this->load->library('pagination');

        $config['base_url'] = base_url().'feedback_employee/page/';
        ...
        $post = $this->feedback_employee_m->get_feedback_employee_list($config['per_page'], $offset, $base_where);
    ...
    public function create(){
        $this->_data['apply'] = $this->user_m->get_all();
        $this->_data['department'] = $this->department_m->get_all();
        $post = new stdClass();
        ...
        if ($this->form_validation->run())
        {
            ...
            if ($id = $this->streams->entries->insert_entry($_POST, 'feedback_employee', 'feedback_employees', array('created'), $extra)) {
                ...
                $this->session->set_flashdata('success', sprintf($this->lang->line('feedback_employee:post_add_success'), $this->input->post('title')));
                Events::trigger('post_created', $id);
            } else {
                $this->session->set_flashdata('error', lang('feedback_employee:post_add_error'));
            }

            ($this->input->post('btnAction') == 'save_exit')
                ? redirect('feedback_employee')
                : redirect('feedback_employee/edit/'.$id);
        ...
        $this->template
            ->title($this->module_details['name'], lang('feedback_employee:create_title'))
    ...
    public function edit($id = 0){
        $this->_data['apply'] = $this->user_m->get_all();
        $this->_data['department'] = $this->department_m->get_all();
        $id or redirect('feedback_employee');
        ...
        if ($this->form_validation->run())
        {
            ...
            if ($this->streams->entries->update_entry($id, $_POST, 'feedback_employee', 'feedback_employees', array('updated'), $extra)) {
                $this->session->set_flashdata(array('success' => sprintf(lang('feedback_employee:edit_success'), $this->input->post('title'))));
                Events::trigger('post_updated', $id);
            } else {
                $this->session->set_flashdata('error', lang('feedback_employee:edit_error'));
            }
        ...
        $this->template
            ->title($this->module_details['name'], sprintf(lang('feedback_employee:edit_title_label'), $post->title))
    ...
    public function delete($id = 0)
    {
        ...
        if (!empty($post_titles)){
            if (count($post_titles) == 1) {
                $this->session->set_flashdata('success', sprintf($this->lang->line('feedback_employee:delete_success'), $post_titles[0]));
            } else {
                $this->session->set_flashdata('success', sprintf($this->lang->line('feedback_employee:mass_delete_success'), implode('", "', $post_titles)));
            }
        } else {
            $this->session->set_flashdata('notice', lang('feedback_employee:delete_error'));
        }
        redirect('feedback_employee');
    }
    ...
    public function manage()
    {
        ...
        $total_rows = $this->feedback_employee_m->count_by($base_where);
        $pagination = create_pagination('feedback_employee/manage/index', $total_rows);

        $post = $this->feedback_employee_m->get_feedback_employee_list($pagination['limit'], $pagination['offset'], $base_where);
