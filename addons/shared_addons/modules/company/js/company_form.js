...
function form_reset(){
    $('#tab_form a').html('Create new Company');
    $('#btnSubmit').html('Create Company');
    $('#title').val("");
    $('#description').val("");
    $('#address').val("");
}
...
function edit_record(id){
    ...
    $('#action').val('edit');
    $('#tab_form a').html('Edit Company');
    $('#btnSubmit').html('Edit Company');
    //Bidding data
    $.ajax({
        type: "POST",
        url: BASE_URL + "company/get_company_by_id/"+id,
        data: {},
        success: function(data){
            var response = $.parseJSON(data);
            $('#title').val(response.title);
            $('#description').val(response.description);
            $('#address').val(response.description);
            //Show tab form team
