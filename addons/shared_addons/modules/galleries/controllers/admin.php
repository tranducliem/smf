...
class Admin extends Admin_Controller
{
    ...
    public function manage($id)
    {
        ...
        $thumbnails = $this->gallery_image_m->where('gallery_images.gallery_id', $id)->dropdown2('files.id', 'files.name');
    ...
    public function refresh($id){
        $id or redirect('admin/galleries');
        $this->load->model(array('files/file_m'));
        //get info gallery and folder id
        $gallery_info = $this->gallery_m->get($id);
        $folder_id = $gallery_info->folder_id;
        $images = $this->file_m->get_file_by_folder($folder_id);
        if($images != null){
            $this->gallery_image_m->delete_images_by_gallery($gallery_info->id);
            $count = 1;
            foreach($images as $item){
                ...
                $this->gallery_image_m->insert($extra);
                $count++;
            }
        }
        $this->session->set_flashdata('success', lang('galleries.refresh_success'));
        redirect('admin/galleries');
    }
}
