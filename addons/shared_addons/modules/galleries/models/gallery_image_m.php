...
class Gallery_image_m extends MY_Model
{
	 ...
	 * @return mixed
	 */
	public function dropdown2($key, $value)
	{
		$dropdown = array();
    ...
    public function get_images_by_gallery_slug($slug, $options = array())
    {
        // Find new images on files
        //$this->set_new_image_files($id);
        ...
        $images = $this->db
            // Select fields on gallery images table
            ->select('gallery_images.*, files.name, files.filename, files.extension, files.description, files.path as path, files.alt_attribute as alt, files.name as title, galleries.folder_id, galleries.slug as gallery_slug, galleries.title as gallery_title')
            // Set my gallery by id
            //->where('galleries.id', $id)
    ...
    public function delete_images_by_gallery($galleryId){
        $query = $this->db->query("DELETE FROM `default_gallery_images` WHERE `default_gallery_images`.`gallery_id` = ".$galleryId);
        return $query;
    }

    //Phan trang client photo
    public function list_all_photo($offset, $start)
    {
        // Grand finale, do what you gotta do!!
        $images = $this->db
            // Select fields on gallery images table
            ...
            // Set my gallery by id
            //->where('galleries.id', $id)
            ->where('galleries.slug', 'gallery-photos')
            // Filter images from my gallery
            ->join('galleries', 'galleries.id = gallery_images.gallery_id', 'left')
            // Filter from my images
            ->join('files', 'files.id = gallery_images.file_id', 'left')
